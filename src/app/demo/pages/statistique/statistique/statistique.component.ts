      ...
      .subscribe((reponse: IResponse) => {
        if (reponse.success) {
          ...
          reponse.data.chiffreMonth.forEach((element) => {
            ...
            datesMois.push(
              element._id.year + " " + this.month[element._id.month - 1]
            );
          });
            ...
            .subscribe((reponse: IResponse) => {
              ...
              for (var i = 0; i < datesMois.length; i++) {
                if (!this.datesMitambatra.includes(datesMois[i])) {
                  ...
                  var indexDate = this.datesMitambatra.findIndex(
                    (x) => x == datesMois[i]
                  );
              ...
              for (var i = 0; i < this.datesMitambatra.length; i++) {
                console.log(
                  this.chiffreMoisMitambatra[i] +
                    "-" +
                    this.depenseMoisMitambatra[i]
                );
                this.beneficeMoisMitambatra[i] =
                  this.chiffreMoisMitambatra[i] - this.depenseMoisMitambatra[i];
              }

              //Triage
              this.statistiqueService.trierDatesMonths(
                this.datesMitambatra,
                this.chiffreMoisMitambatra,
                this.depenseMoisMitambatra,
                this.beneficeMoisMitambatra
              );
